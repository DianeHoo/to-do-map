...
function deleteDumpTask(id) {
  ...
  state.urgencyOrder = state.urgencyOrder.filter(x => x !== id);
  state.importanceOrder = state.importanceOrder.filter(x => x !== id);
...
function computeCanvasPositions(canvasW, canvasH) {
  ...
  state.tasks.forEach(task => {
    ...
    const urgencyNorm = urgencyRank === -1 ? 0.5 : (n > 1 ? urgencyRank / (n - 1) : 0.5);
    const importanceNorm = importanceRank === -1 ? 0.5 : (n > 1 ? importanceRank / (n - 1) : 0.5);
...
function loadSavedState() {
  try {
    ...
    // Data written by the impact/effort variant under the legacy shared key —
    // not ours to load; that page migrates it to its own key on next visit.
    if ('impactOrder' in saved) return false;
    ...
    state.urgencyOrder = state.urgencyOrder.filter(id => liveIds.has(id));
    state.importanceOrder = state.importanceOrder.filter(id => liveIds.has(id));
...
function doCleanup() {
  ...
  const undoData = {
    ...
    urgencyOrder: state.urgencyOrder.slice(),
    importanceOrder: state.importanceOrder.slice(),
    cardPositions: { ...state.cardPositions },
    scatterSig: state.scatterSig,
  };
...
function parseGridFile(text) {
  ...
  // renderSortList() draws one card per entry in urgencyOrder/importanceOrder,
  // keyed by id with no dedup of its own — a repeated id in an untrusted
  // payload survives the ids.has() check twice and renders as two identical
  // cards for the same task. Worse, onSortKeyDown's `order.indexOf(id)`
  // always resolves to the *first* copy, so arrow-key reordering on the
  // second (visually identical) card silently moves the wrong one. Keep
  // only the first occurrence of each id, same as the shape checks above
  // reject the rest of a bad entry instead of failing the whole import.
  const idList = (v) => {
  ...
  return {
    ...
    importanceOrder: idList(g.importanceOrder),
...
function importGrid(file) {
  ...
  reader.onload = function(e) {
    ...
    state.urgencyOrder = candidate.urgencyOrder;
    state.importanceOrder = candidate.importanceOrder;
...
const SHARE_MAP_KIND = 'urgency-importance';
